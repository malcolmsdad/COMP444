...
float detectDistance = 13;     // variable to store limit before turning around
const int spinTimes = 5;
...
void loop()
{
  ...
  if (digitalRead(switchPin) == LOW) { //if the on switch is flipped
    ...
    if (distance < detectDistance) {              //if an object is detected
      //back up and turn
      Serial.print(" ");
      Serial.print("BACK!");
...
void rightMotor(int motorSpeed)                       //function for driving the right motor
{
  ...
  analogWrite(PWMA, abs(motorSpeed) * 0.90);           // HAcK Reduce GAIN due to misaligned wheels
